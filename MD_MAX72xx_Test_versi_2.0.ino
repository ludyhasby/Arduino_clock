...
{
#if 1
  scrollText("Nothing Is Impossible");
  cross();
  scrollText("Stand up! Those who refuse to be slaves!,");
  scrollText(" With our flesh and blood, let's build our newest Great Wall!");
  bullseye();
  scrollText("YZX - YS");
  spiral();
#endif
...
#if 1
  scrollText("Σλ Plan 03-23");
  scanLimit();
  scrollText("12.15-13.15 -> SN220");
  scrollText("13.15-13.30 -> dhuhuran");
  scrollText("13.30-15.30 ->working esp, ekmon, makro1");
  scrollText("16.00 - 18.30 -> kp ekint ");
  stripe();
  scrollText("18.30-21.00 -> nderes, tarawih");
  transformation2();
  scrollText("21.30-22.30 -> learning 1");
  scrollText("21.30-22.30 -> learning 2");
  scrollText("22.30 - 0.30 -> snbt");
  blinking();
#endif

#if 1
  scrollText("List must doing -> Kuis Ekint (dead::sunday)");
  scrollText("Makro1 (dead:: 29");
  scrollText("mikro2 (third)");
  scrollText("ekmon (pr) dead:: 28");
  transformation1();
#endif
}
